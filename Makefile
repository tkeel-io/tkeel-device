...
GORUN = $(GOCMD) run -ldflags "-X github.com/tkeel-io/tkeel-device/cmd.GitCommit=${GIT_COMMIT}${GIT_DIRTY} -X github.com/tkeel-io/tkeel-device/cmd/cmd.BuildDate=${BUILD_DATE}"
GOBUILD = $(GOCMD) build -ldflags "-X github.com/tkeel-io/tkeel-device/cmd.GitCommit=${GIT_COMMIT}${GIT_DIRTY} -X github.com/tkeel-io/tkeel-device/cmd/cmd.BuildDate=${BUILD_DATE}"
GOTEST = $(GOCMD) test
BINNAME = tkeel-device
...
run:
	...
	@echo "-         Run             -"
	@echo "---------------------------"
	@$(GORUN) cmd/device/main.go serve

drun:
	dapr run --app-id tkeel-device --app-protocol http --app-port 6789 --dapr-http-port 3500 --dapr-grpc-port 50001 --log-level debug  --components-path ./examples/configs/tkeel-device  go run cmd/device/main.go
...
# Go build details                                                             #
################################################################################
BASE_PACKAGE_NAME := github.com/tkeel-io/tkeel-device
OUT_DIR := ./dist

BINS_OUT_DIR := $(OUT_DIR)/$(GOOS)_$(GOARCH)/$(BUILDTYPE_DIR)
LDFLAGS := "-X github.com/tkeel-io/tkeel-device/cmd.GitCommit=${GIT_COMMIT}${GIT_DIRTY} -X github.com/tkeel-io/tkeel-device/cmd/cmd.BuildDate=${BUILD_DATE}"
...
build:
	...
	@CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build $(GCFLAGS) -ldflags $(LDFLAGS) \
     	-o $(BINS_OUT_DIR)/$(CLI_BINARY)$(BINARY_EXT) cmd/device/main.go
...
docker-build: build
	docker build -t tkeelio/tkeel-device:0.4.0-alpha.3 .
docker-push:
	docker push tkeelio/tkeel-device:0.4.0-alpha.3
################################################################################
# Target: lint                                                                 #
################################################################################
